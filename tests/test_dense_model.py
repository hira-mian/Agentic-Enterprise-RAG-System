...
@pytest.mark.skipif(os.environ.get('RUN_MODEL_TESTS') != '1', reason='Set RUN_MODEL_TESTS=1 after downloading BGE')
def test_real_bge_semantic_search_reload_and_scope(tmp_path):
    ...
    docs=[Document.model_validate_json(line) for line in fixture.read_text().splitlines()]
    ...
    assert [hit.score for hit in restored]==pytest.approx([hit.score for hit in result])
