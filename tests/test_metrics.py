...
from src.evaluation.retrieval_metrics import recall_at_k, ndcg_at_k, evaluate_retrieval


def test_hand_calculated_ranking_and_deduplication():
    # Unique ranking is a, irrelevant, b. Both a and b are relevant.
    hits=['a','a','x','b']
    assert recall_at_k(hits,['a','b'],2)==.5
...
def test_aggregation_includes_errors_and_reports_exclusions():
    rows=[{'question_id':'1','question_type':'basic','retrieved_doc_ids':['a'],'relevant_doc_ids':['a']},
          {'question_id':'2','question_type':'basic','retrieved_doc_ids':['a'],'relevant_doc_ids':['a'],'error':'timeout'},
          {'question_id':'3','question_type':'unknown','retrieved_doc_ids':[],'relevant_doc_ids':[]}]
    report=evaluate_retrieval(rows,ks=(10,))
    assert report['overall']['recall@10']=={'mean':.5,'scored':2,'excluded':1}
