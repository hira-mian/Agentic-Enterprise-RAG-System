...
def main():
    ...
    if args.action == "build":
        ...
        provenance = {"selected_input_sha256":digest.hexdigest(), "documents":len(documents),
        ...
        if args.method == "bm25":
            ...
            index = DenseIndex(chunks,BGEEncoder(EMBEDDING_REVISION),provenance=provenance)
        index.save(args.output)
        print(json.dumps({"documents":len(documents), "chunks":len(chunks), "method":args.method}))
    else:
        ...
        if manifest["method"] == "bm25":
            ...
            from src.retrieval.dense import BGEEncoder, DenseIndex
            encoder = BGEEncoder(manifest["config"]["revision"],model_name=manifest["config"]["name"])
            index = DenseIndex.load(args.index,encoder)
        else:
            parser.error("Unsupported index method")
        ids = {c.doc_id for c in index.chunks} if args.benchmark_all_docs else set(args.allow_doc)
        request = SearchRequest(query=args.query,top_k=args.top_k,
                                user=UserContext(user_id="local-benchmark",allowed_doc_ids=ids))
        print(json.dumps([{"doc_id":e.chunk.doc_id, "chunk_id":e.chunk.chunk_id,
                           "score":e.score,"excerpt":e.chunk.text[:200]} for e in index.search(request)],indent=2))
