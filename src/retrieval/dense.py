...
from src.retrieval.common import eligible, evidence_for, load_metadata, save_metadata, validate_chunks
...
class BGEEncoder:
    def __init__(self, revision: str, model_name="BAAI/bge-small-en-v1.5", batch_size=32):
    ...
    @property
    def identity(self):
        return {"name":self.model_name, "revision":self.revision, "query_prefix":self.prefix,
                "normalize":True, "max_seq_length":self.model.max_seq_length}

    def encode(self, texts, *, query=False):
        inputs = [self.prefix+t if query else t for t in texts]
        lengths = self.model.tokenizer(inputs, truncation=False, return_length=True)["length"]
        if any(length > self.model.max_seq_length for length in lengths):
            raise ValueError("Input exceeds embedding token limit; reduce chunk/query length")
        return self.model.encode(inputs, batch_size=self.batch_size, normalize_embeddings=True,
...
class DenseIndex:
    def __init__(self, chunks, encoder: Encoder, vectors=None, provenance=None):
        ...
        self.vectors = normalized(encoder.encode([c.text for c in self.chunks]) if vectors is None else vectors)
    ...
    def save(self, directory):
        path = Path(directory)
        manifest = save_metadata(path, self.chunks, "dense", self.encoder.identity, self.provenance)
        faiss.write_index(self.index, str(path / "index.faiss"))
        manifest["index_sha256"] = sha256((path / "index.faiss").read_bytes()).hexdigest()
    ...
    def load(cls, directory, encoder):
        ...
        if sha256((path / "index.faiss").read_bytes()).hexdigest() != manifest["index_sha256"]:
        ...
        return cls(chunks, encoder, index.reconstruct_n(0,index.ntotal), manifest["provenance"])
