"""Source-grounded generation with schema/citation validation, not semantic judging."""
...
class GenerationConfig(Contract):
    ...
    retry_delay_seconds: float = Field(default=.5, ge=0, le=10)


def combined_usage(records):
    known = all(u.estimated_cost_usd is not None for u in records)
    return Usage(input_tokens=sum(u.input_tokens for u in records),
                 output_tokens=sum(u.output_tokens for u in records),
                 api_calls=sum(u.api_calls for u in records),
                 estimated_cost_usd=sum(u.estimated_cost_usd for u in records) if known else None)
...
class GroundedGenerator:
    ...
    def generate(self, request: GenerationRequest) -> tuple[Answer, Usage]:
        ...
        if len(request.question) > self.config.max_question_chars:
            return Answer(status="error", text="Question exceeds the configured length limit."), zero
        selected = select_context(request, self.config.max_context_chars)
        if not selected:
            return Answer(status="insufficient_evidence", text="No authorized evidence fits the context budget."), zero
        prompt = json.dumps({"question":request.question,
                             "evidence":[{"citation_id":e.citation_id,
        ...
        for attempt in range(self.config.retries + 1):
            try:
                ...
                if error.retryable and attempt < self.config.retries:
                    ...
                    continue
                return Answer(status="error", text="Answer service unavailable or request budget exhausted."), combined_usage(records)
            ...
            try:
                ...
                # Structural failure: do not turn malformed text into a confident answer.
                return Answer(status="error", text="Answer service returned an invalid response."), combined_usage(records)
            return answer, combined_usage(records)
        raise AssertionError("Unreachable")


class FakeProvider:
    """Scripted offline provider for fixtures; not an LLM or quality baseline."""
...
class AnthropicConfig(Contract):
    ...
    def paid_rates(self):
        if self.allow_paid and min(self.budget_usd, self.input_usd_per_million,
                                   self.output_usd_per_million) <= 0:
            raise ValueError("Paid calls require a positive budget and explicit token prices")
        return self
...
class AnthropicProvider:
    ...
    def complete(self, system, prompt):
        ...
        if self.client is None:
            ...
            try:
                self.client = anthropic.Anthropic(max_retries=0, timeout=config.timeout_seconds)
        ...
        try:
            calls += 1
            count = self.client.messages.count_tokens(model=config.model, system=system, messages=messages)
            reservation = (count.input_tokens*config.input_usd_per_million
                           + config.max_output_tokens*config.output_usd_per_million) / 1_000_000
            ...
            message = self.client.messages.create(model=config.model, max_tokens=config.max_output_tokens,
                                                   system=system, messages=messages, temperature=0)
        except (anthropic.APIError, anthropic.APITimeoutError) as error:
            status = getattr(error, "status_code", None)
            retryable = status in (408,409,429) or (status is not None and status >= 500) or isinstance(error, anthropic.APIConnectionError)
            raise ProviderError(retryable=retryable,
                                usage=Usage(api_calls=calls, estimated_cost_usd=None if generation_started else 0)) from None
        ...
        return Completion(text, Usage(input_tokens=input_tokens, output_tokens=output_tokens,
                                      api_calls=calls, estimated_cost_usd=cost),
